...
export default function Switcher() {
  ...
  return (
    <div className="flex items-center justify-center gap-2">
      <Tooltip content="효과음 설정">
        <SoundSwitcher />
      </Tooltip>

      <Tooltip className="h-7 w-7" content="단어 반복 설정">
        <LoopWordSwitcher />
      </Tooltip>

      <Tooltip className="h-7 w-7" content={`암기 모드（${CTRL} + V）`}>
        <WordDictationSwitcher />
      </Tooltip>
      <Tooltip className="h-7 w-7" content={`번역 표시（${CTRL} + Shift + V）`}>
        <button
          ...
          aria-label={`번역 표시（${CTRL} + Shift + V）`}
        >
          {state?.isTransVisible ? <IconLanguage /> : <IconLanguageOff />}
        </button>
      </Tooltip>

      <Tooltip content="틀린 단어">
        <ErrorBookButton />
      </Tooltip>

      <Tooltip className="h-7 w-7" content="통계 데이터 보기">
        <AnalysisButton />
      </Tooltip>

      <Tooltip className="h-7 w-7" content="어두운 모드">
        <button
