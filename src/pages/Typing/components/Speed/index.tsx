...
export default function Speed() {
  ...
  return (
    <div className="my-card flex w-3/5 rounded-xl bg-white p-4 py-10 opacity-50 transition-colors duration-300 dark:bg-gray-800">
      <InfoBox info={`${minutesString}:${secondsString}`} description="시간" />
      <InfoBox info={inputNumber + ''} description="입력" />
      <InfoBox info={state.timerData.wpm + ''} description="WPM" />
      <InfoBox info={state.chapterData.correctCount + ''} description="정확" />
      <InfoBox info={state.timerData.accuracy + ''} description="정확률" />
    </div>
  )
}
